...
def profile_query_fun(tx, query_spec, query_parameters):
    profile_result = tx.run("PROFILE " + query_spec, query_parameters)
...
def plan_to_dict(plan):
    return {
        "operatorType": plan.get("operatorType"),
        "identifiers": plan.get("identifiers", []),
        "dbHits": plan.get("dbHits", 0),
        "rows": plan.get("rows", 0),
        "pageCacheHits": plan.get("pageCacheHits", 0),
        "pageCacheMisses": plan.get("pageCacheMisses", 0),
        "args": plan.get("args", {}),
        "children": [plan_to_dict(child) for child in plan.get("children", [])]
    }
def run_query(session, query_num, query_variant, query_spec, query_parameters, test, profile_file_path=None):
    if test:
    ...
    if profile_file_path:
        profile_output = session.write_transaction(profile_query_fun, query_spec, query_parameters)
        with open(profile_file_path, 'w') as pf:
            pf.write(profile_output)
...
#           print(perf)
           ...
           # (results, duration) = run_query(session, query_num, query_variant, query_spec, query_parameters_converted, test)
            ...
            (results, duration) = run_query(session, query_num, query_variant, query_spec,query_parameters_converted, test, profile_file_path=profile_file)

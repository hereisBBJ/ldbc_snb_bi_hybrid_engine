...
import time
import json
from queries_pr import run_queries, run_precomputations

query_variants = ["1", "2a", "2b", "3", "4", "5", "6", "7", "8a", "8b", "9",
                  "10a", "10b", "11", "12", "13", "14a", "14b",
                  "16a", "16b", "17", "18", "19a", "19b", "20a", "20b"]
# "15a","15b"
# python3 benchmark.py --scale_factor 1 --container_name my_container --test
...
output = Path(f"output_pr/output-sf{sf}")
output.mkdir(parents=True, exist_ok=True)
open(f"output_pr/output-sf{sf}/results.csv", "w").close()
open(f"output_pr/output-sf{sf}/timings.csv", "w").close()
timings_file = open(f"output_pr/output-sf{sf}/timings.csv", "a")
timings_file.write("tool|sf|day|batch_type|q|parameters|time\n")
results_file = open(f"output_pr/output-sf{sf}/results.csv", "a")
...
if queries_only:
    run_precomputations(query_variants, batch_date, batch_type, sf, timings_file)
...
with open(f"output_pr/output-sf{sf}/benchmark.csv", "w") as bf:
    bf.write("time\n")

...
SET VARIABLE country1 = 'Angola';
SET VARIABLE country2 = 'Slovakia';
...
WITH PersonPairCandidates AS (
    SELECT Person1.id AS Person1Id
      ...
      JOIN undirected_Person_knows_Person
        ON undirected_Person_knows_Person.Person1Id = Person1.id
      JOIN Person Person2
        ON Person2.id = undirected_Person_knows_Person.Person2Id
      JOIN City City2
        ON Person2.LocationCityId = City2.id
      JOIN Country Country2
        ON Country2.id = City2.PartOfCountryId
     WHERE Country1.name = getvariable('country1')
       AND Country2.name = getvariable('country2')
)

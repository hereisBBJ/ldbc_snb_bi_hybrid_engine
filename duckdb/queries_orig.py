...
def run_query(query_num, query_variant, query_spec, query_parameters, test, perf_file, phase_timings_dir=None):
    # BI-15 走 pybind11/C++ 加速分支（见 bi15/pybind_backend.py）
    if query_num == 15:
        return run_query_15(query_variant, query_parameters, perf_file, phase_timings_dir=phase_timings_dir)

    # BI-19 走 DuckDB + igraph 分支：SQL 建图，igraph 做最短路
    if query_num == 19:
        return run_query_19(query_variant, query_spec, query_parameters, perf_file, phase_timings_dir=phase_timings_dir)

    # BI-20 走 DuckDB + igraph 分支：SQL 建图，igraph 做最短路
    if query_num == 20:
        return run_query_20(query_variant, query_spec, query_parameters, perf_file, phase_timings_dir=phase_timings_dir)
...
def run_precomputations(query_variants, batch_id, batch_type, sf, timings_file,
                        use_precomputed_paths=True, perf_base_dir='system_performance_orig'):
    if "4" in query_variants:
        perf_file_path = perf_base_dir
...
def run_queries(query_variants, parameter_csvs, sf, test, pgtuning, batch_id, batch_type,
                timings_file, results_file, use_precomputed_paths=True,
                phase_timings_dir=None, perf_base_dir='system_performance_orig'):
    ...
    for query_variant in query_variants:
        ...
        for query_parameters in parameters_csv:
            i += 1
            perf_file_path = f'{perf_base_dir}/bi-{query_num}{query_subvariant}'
            os.makedirs(perf_file_path, exist_ok=True)
            perf_file = f'{perf_file_path}/parameters-{i}.csv'
            # DuckDB 执行
            results, duration = run_query(
                query_num, query_variant, query_spec, query_parameters, test, perf_file,
                phase_timings_dir=phase_timings_dir
            )

...
duckdb_path = 'duckdb-pr/duckdb/build/release/duckdb'
db_file = 'duckdb-pr/duckdb/import_data/ldbc_db.duckdb'
...
def run_precomputations(query_variants, batch_id, batch_type, sf, timings_file):
    if "4" in query_variants:
        perf_file_path = f'system_performance_pr'
        ...
        timings_file.write(f"DuckDB-PR|{sf}|{batch_id}|{batch_type}|q4precomputation||{duration}\n")
    if "6" in query_variants:
        ...
        timings_file.write(f"DuckDB-PR|{sf}|{batch_id}|{batch_type}|q6precomputation||{duration}\n")
    if "19a" in query_variants or "19b" in query_variants:
        ...
        timings_file.write(f"DuckDB-PR|{sf}|{batch_id}|{batch_type}|q19precomputation||{duration}\n")
    if "20a" in query_variants or "20b" in query_variants:
        ...
        timings_file.write(f"DuckDB-PR|{sf}|{batch_id}|{batch_type}|q20precomputation||{duration}\n")
...
def run_queries(query_variants, parameter_csvs, sf, test, pgtuning,batch_id, batch_type, timings_file, results_file):
    ...
    for query_variant in query_variants:
        ...
        for query_parameters in parameters_csv:
            i += 1
            perf_file_path = f'system_performance_pr/bi-{query_num}{query_subvariant}'
            os.makedirs(perf_file_path, exist_ok=True)
            ...
            timings_file.write(f"DuckDB-PR|{sf}|{batch_id}|{batch_type}|{query_variant}|{query_parameters_json}|{duration}\n")
            timings_file.flush()

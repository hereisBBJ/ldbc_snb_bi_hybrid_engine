...
SET VARIABLE tag = 'Che_Guevara';
SET VARIABLE startDate = DATE '2011-07-20';
SET VARIABLE endDate = DATE '2011-07-25';
 */
   ...
   , Person_Message_score AS (
    SELECT Person.id AS PersonId
      ...
      JOIN Message
       ...
       AND getvariable('startDate') < Message.creationDate
      JOIN Person
        ON Person.id = Message.CreatorPersonId
     WHERE Tag.name = getvariable('tag')
       AND Message.creationDate < getvariable('endDate')
     GROUP BY Person.id
)
   , Person_score AS (
...
SELECT p.PersonId AS "person.id"
  ...
  LEFT JOIN undirected_Person_knows_Person
    ON undirected_Person_knows_Person.Person1Id = p.PersonId
  LEFT JOIN Person_score f -- the friend
    ON f.PersonId = undirected_Person_knows_Person.Person2Id

/* Q19. Interaction path between cities (full mode: build path weights in-query)
\set city1Id 608
\set city2Id 1148

SET VARIABLE city1Id = 608;
SET VARIABLE city2Id = 1148;
 */
with recursive
PathQ19(src, dst, w) as (
    with
...
srcs(f) as (select id from Person where locationcityid = getvariable('city1Id')),
dsts(t) as (select id from Person where locationcityid = getvariable('city2Id')),

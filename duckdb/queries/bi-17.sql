...
SET VARIABLE tag = 'Slavoj_Žižek';
SET VARIABLE delta = 4;
 */
WITH MyMessage as (
    SELECT *
    FROM Message
-- (tag)<-[:HAS_TAG]-(message)
    WHERE MessageId in (SELECT MessageId FROM Message_hasTag_Tag WHERE TagId IN (SELECT id FROM Tag WHERE Tag.name = getvariable('tag')))
)
...
JOIN MyMessage Message2
 ON (Message1.creationDate + (getvariable('delta') || ' hours')::INTERVAL) < Message2.creationDate

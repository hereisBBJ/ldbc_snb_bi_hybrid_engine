...
SET VARIABLE date = DATE '2012-06-01';
SET VARIABLE tagClass = 'MusicalArtist';
 */
WITH
MyTag AS (
...
SELECT t.id as TagId
     , count(CASE WHEN Message.creationDate <  getvariable('date') + INTERVAL '100 days' THEN Message.MessageId ELSE NULL END) AS countMonth1
     , count(CASE WHEN Message.creationDate >= getvariable('date') + INTERVAL '100 days' THEN Message.MessageId ELSE NULL END) AS countMonth2
  FROM MyTag t
  JOIN Message_hasTag_Tag
         ON Message_hasTag_tag.TagId = t.id
  JOIN Message
    ON Message.MessageId = Message_hasTag_tag.MessageId
   AND Message.creationDate >= getvariable('date')
   AND Message.creationDate <  getvariable('date') + INTERVAL '200 days'
 GROUP BY t.id
)

...
SET VARIABLE startDate = DATE '2011-10-01';
SET VARIABLE endDate = DATE '2011-10-15';
 */
WITH
MPP AS (SELECT RootPostId, count(*) as MessageCount FROM Message WHERE Message.creationDate BETWEEN getvariable('startDate') AND getvariable('endDate') GROUP BY RootPostId)
SELECT Person.id AS "person.id"
 ...
 WHERE Post.creationDate BETWEEN getvariable('startDate') AND getvariable('endDate')

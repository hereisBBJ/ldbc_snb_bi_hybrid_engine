...
SET VARIABLE endDate = DATE '2013-01-01';
...
WITH Zombies AS (
    SELECT Person.id AS zombieid
      ...
      LEFT JOIN Message
        ...
        AND Message.creationDate BETWEEN Person.creationDate AND getvariable('endDate') -- the lower bound is an optmization to prune messages
     WHERE Country.name = getvariable('country')
       AND Person.creationDate < getvariable('endDate')
    ...
    HAVING count(Message.MessageId) < 12*extract(YEAR FROM getvariable('endDate')) + extract(MONTH FROM getvariable('endDate'))
...
SELECT Z.zombieid AS "zombie.id"
  ...
  FROM Zombies Z LEFT JOIN (
    ...
    WHERE Z.zombieid = m.CreatorPersonId AND p.creationDate < getvariable('endDate')

...
37383395347825|10995116283851|2012-11-05|2012-11-07

SET VARIABLE person1Id = 37383395347825;
SET VARIABLE person2Id = 10995116283851;
SET VARIABLE startDate = DATE '2012-11-05';
SET VARIABLE endDate = DATE '2012-11-07';

15393162808658|28587302357897|2010-04-11|2010-05-03

SET VARIABLE person1Id = 15393162808658;
SET VARIABLE person2Id = 28587302357897;
SET VARIABLE startDate = DATE '2010-04-11';
SET VARIABLE endDate = DATE '2010-05-03';
 */
with recursive
srcs(f) as (select getvariable('person1Id')),
dsts(t) as (select getvariable('person2Id')),
myForums(id) as (
    select id from Forum f where f.creationDate between getvariable('startDate') and getvariable('endDate')
),
mm as (
...
path(src, dst, w) as (
    ...
    from undirected_Person_knows_Person pp left join mm on least(pp.person1id, pp.person2id) = mm.src and greatest(pp.person1id, pp.person2id) = mm.dst
),

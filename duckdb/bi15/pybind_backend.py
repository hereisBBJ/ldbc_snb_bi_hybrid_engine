"""
bi15/pybind_backend.py
BI-15 主加速入口（pybind11/C++ 版本）：
  ...
  - bi15_dijkstra_cpp（pybind11 C++ 扩展）负责 Dijkstra 最短路径
    ...
    * numpy 数组直接传裸指针（unchecked<1>），无额外拷贝
  - 若 bi15_dijkstra_cpp 尚未编译，自动回退到 igraph 实现

注意：bi15/__init__.py 已将 bi15/ 目录加入 sys.path，
      因此 bi15_dijkstra_cpp.so 可被直接 import。
"""
...
# ---------------------------------------------------------------------------
# 后端优先级：igraph C++ 扩展 > 纯 C++ 扩展 > Python igraph 回退
# ---------------------------------------------------------------------------
try:
    import bi15_dijkstra_igraph_cpp
    _BACKEND = "igraph_cpp"
except ImportError:
    try:
...
# db_config 位于父目录 duckdb/，运行时 cwd=duckdb/ 故可直接 import
...
def _dijkstra_igraph_cpp(arrow_table, person1Id, person2Id, timings):
    """Arrow Table → numpy → C++(igraph C API) Dijkstra。"""
    if arrow_table.num_rows == 0:
        return -1
    ...
    _t = time.perf_counter()
    dist = bi15_dijkstra_igraph_cpp.dijkstra_shortest_path_igraph(
        src_arr, dst_arr, weight_arr,
        int(person1Id), int(person2Id)
    )
    timings['dijkstra_cpp'] += time.perf_counter() - _t
...
def run_query_15(query_variant, query_parameters, perf_file):
    ...
    使用 pybind11 C++ Dijkstra 加速（如可用），否则回退 igraph。
    """
    ...
    try:
        for _ in range(10):
            ...
            if _BACKEND == "igraph_cpp":
                dist = _dijkstra_igraph_cpp(arrow_table, person1Id, person2Id, timings)
            elif _BACKEND == "pybind11":

...
def run_precomputations(query_variants, pg_con, cur, batch_id, batch_type, sf, timings_file,container_name):
    if "4" in query_variants:
        ...
        perf.kill()
        timings_file.write(f"PostgreSQL|{sf}|{batch_id}|{batch_type}|q4precomputation||{end-start}\n")
    if "6" in query_variants:
        ...
        perf.kill()
        timings_file.write(f"PostgreSQL|{sf}|{batch_id}|{batch_type}|q6precomputation||{end-start}\n")
    if "19a" in query_variants or "19b" in query_variants:
        ...
        perf.kill()
        timings_file.write(f"PostgreSQL|{sf}|{batch_id}|{batch_type}|q19precomputation||{end-start}\n")
    if "20a" in query_variants or "20b" in query_variants:
        ...
        perf.kill()
        timings_file.write(f"PostgreSQL|{sf}|{batch_id}|{batch_type}|q20precomputation||{end-start}\n")

def run_queries(query_variants, parameter_csvs, pg_con, sf, test, pgtuning, batch_id, batch_type, timings_file, results_file,container_name):
    ...
    for query_variant in query_variants:
        ...
        for query_parameters in parameters_csv:
            ...
            profile_file = f'{perf_file_path}/parameters-{i}-profile.json'

            perf = subprocess.Popen(["/d1/machine_performance_indicators/system_perf.sh", perf_file,container_name])
           #print(["/d1/machine_performance_indicators/system_perf.py", perf_file,container_name])
            ...
            # 替换参数
            executable_query = query_spec
            for key in query_parameters_converted.keys():
                executable_query = executable_query.replace(f":{key}", query_parameters_converted[key])

            cur = pg_con.cursor()

            # ==== EXPLAIN (ANALYZE, FORMAT JSON) ====
            explain_query = f"EXPLAIN (ANALYZE, FORMAT JSON) {executable_query}"
            cur.execute(explain_query)
            explain_result = cur.fetchall()

            with open(profile_file, 'w') as f:
                json.dump(explain_result[0][0], f)  # explain_result 是嵌套 JSON 数组

            # ==== 执行实际查询 ====
            start_time = time.time()
            cur.execute(executable_query)
            results = cur.fetchall()
            cur.execute("COMMIT;")
            ...
            timings_file.write(f"PostgreSQL|{sf}|{batch_id}|{batch_type}|{query_variant}|{query_parameters_in_order}|{duration}\n")
            timings_file.flush()
